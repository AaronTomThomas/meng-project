...
"""
Layer-wise bridge between TTR readout improvability and trained PURPLE deltas.
...
attention-adapter checkpoint plumbing for trained PURPLE/AKAZA checkpoints.
"""
...
def summarize_vectors(
    g_ttr: torch.Tensor,
    g_full_model_purple: torch.Tensor,
    delta_norm: torch.Tensor,
    delta_wo_norm: torch.Tensor,
    projection_gain: torch.Tensor,
    full_layer_gain: torch.Tensor,
) -> Dict[str, Any]:
    return {
        "n_token_layer": int(g_ttr.numel()),
        "mean_G_TTR": float(g_ttr.float().mean().item()),
        "mean_G_PURPLE_LAYER": float(full_layer_gain.float().mean().item()),
        "mean_G_FULL_MODEL_PURPLE": float(g_full_model_purple.float().mean().item()),
        "mean_projection_gain": float(projection_gain.float().mean().item()),
        "mean_full_layer_gain": float(full_layer_gain.float().mean().item()),
        ...
        "corr_G_TTR_G_PURPLE_LAYER": spearman_corr(g_ttr, full_layer_gain),
        "corr_G_TTR_G_FULL_MODEL_PURPLE": spearman_corr(g_ttr, g_full_model_purple),
        "corr_G_TTR_projection_gain": spearman_corr(g_ttr, projection_gain),
        ...
        "top_decile_G_PURPLE_LAYER_enrichment": top_decile_enrichment(g_ttr, full_layer_gain),
        "top_decile_projection_gain_enrichment": top_decile_enrichment(g_ttr, projection_gain),
        "top_decile_G_FULL_MODEL_PURPLE_enrichment": top_decile_enrichment(g_ttr, g_full_model_purple),
    }


@torch.no_grad()
def eval_ttr_layerwise_bridge(
    *,
    model,
        ...
        for start in range(0, n_examples, batch_size):
            ...
            for layer_idx in layer_indices:
                ...
                if token_writer is not None:
                    ...
                    full_layer_gain_cpu = full_layer_gain.detach().cpu()
                    projection_gain_cpu = projection_gain.detach().cpu()
                            ...
                            token_writer.writerow(
                                {
                                    ...
                                    "G_FULL_MODEL_PURPLE": float(g_akaza_cpu[b, pos].item()),
                                    "G_PURPLE_LAYER": float(full_layer_gain_cpu[b, pos].item()),
                                    "projection_gain": float(projection_gain_cpu[b, pos].item()),
    ...
    global_summary = summarize_vectors(
        ...
        global_projection_gain,
        global_full_layer_gain,
    )
...
def write_layer_csv(path: str, layers: Dict[str, Any]) -> None:
    fieldnames = [
        ...
        "mean_G_TTR",
        "mean_G_PURPLE_LAYER",
        "mean_G_FULL_MODEL_PURPLE",
        "corr_G_TTR_G_PURPLE_LAYER",
        "corr_G_TTR_G_FULL_MODEL_PURPLE",
        "corr_G_TTR_projection_gain",
        "projection_energy_over_delta_energy",
        "projection_gain_over_full_layer_gain",
        "mean_projection_gain",
        "mean_full_layer_gain",
    ]
    out = Path(path)
            ...
            writer.writerow(
                {
                    ...
                    "mean_G_PURPLE_LAYER": metrics["mean_G_PURPLE_LAYER"],
                    "mean_G_FULL_MODEL_PURPLE": metrics["mean_G_FULL_MODEL_PURPLE"],
                    "corr_G_TTR_G_PURPLE_LAYER": metrics["corr_G_TTR_G_PURPLE_LAYER"],
                    "corr_G_TTR_G_FULL_MODEL_PURPLE": metrics["corr_G_TTR_G_FULL_MODEL_PURPLE"],
                    "corr_G_TTR_projection_gain": metrics["corr_G_TTR_projection_gain"],
                    ...
                    "mean_projection_gain": recovery["mean_projection_gain"],
                    "mean_full_layer_gain": recovery["mean_full_layer_gain"],
                }
            )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Evaluate the layer-wise TTR bridge for GPT-2 PURPLE/AKAZA checkpoints.")
    ...
    metrics = eval_ttr_layerwise_bridge(
        model=model,
        wrapped=wrapped,
        chunks=chunks,
    ...
    results: Dict[str, Any] = {
        ...
        "bridge_definition": {
            ...
            "G_PURPLE_LAYER": "base_token_nll - token_nll_after_replacing_layer_z_with z_soft + trained_delta_for_that_same_layer",
            "G_FULL_MODEL_PURPLE": "base_token_nll - token_nll_after_full_trained_PURPLE_forward",
            "main_metric": "corr_G_TTR_G_PURPLE_LAYER",
        },
        **metrics,
    }
    ...
    print("[summary]")
    for layer_idx, layer_metrics in sorted(metrics["layers"].items(), key=lambda kv: int(kv[0])):
        recovery = layer_metrics["projection_recovery"]
        print(
            f"  layer {int(layer_idx)}: "
            f"corr(G_TTR, G_PURPLE_LAYER)={layer_metrics['corr_G_TTR_G_PURPLE_LAYER']:.6f}, "
            f"mean_G_TTR={layer_metrics['mean_G_TTR']:.6f}, "
            f"mean_G_PURPLE_LAYER={layer_metrics['mean_G_PURPLE_LAYER']:.6f}, "
            f"projection/full={recovery['projection_gain_over_full_layer_gain']:.6f}"
        )
    global_recovery = metrics["global"]["projection_recovery"]
    print(
        f"  global: "
        f"corr(G_TTR, G_PURPLE_LAYER)={metrics['global']['corr_G_TTR_G_PURPLE_LAYER']:.6f}, "
        f"mean_G_TTR={metrics['global']['mean_G_TTR']:.6f}, "
        f"mean_G_PURPLE_LAYER={metrics['global']['mean_G_PURPLE_LAYER']:.6f}, "
        f"projection/full={global_recovery['projection_gain_over_full_layer_gain']:.6f}"
    )
    print(f"  wrote: {out}")
