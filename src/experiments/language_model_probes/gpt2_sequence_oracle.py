...
def apply_sequence_assignment(
    ...
    """
    Apply a per-position learner assignment to a single cached sequence.

    This script evaluates one packed text chunk at a time, so the batch size is
    intentionally one. The assertion prevents silent misuse with larger batches.
    """
    if zcat_teacher.shape[0] != 1:
        raise ValueError(
            "apply_sequence_assignment assumes batch size 1. "
            f"Got zcat_teacher shape {tuple(zcat_teacher.shape)}."
        )
...
@torch.no_grad()
def validate_noop_reconstruction(
    model,
    ...
    """
    Check that continuing the model from the reconstructed pre-c_proj attention
    output reproduces the original GPT-2 logits.
    """
    full_logits = model(input_ids).logits

    recon_logits = continue_from_modified_block_gpt2(
        model=model,
        block=block,
        x_in=x_in,
        zcat_mod=zcat_teacher,
        layer_idx=layer_idx,
    )

    diff = (full_logits - recon_logits).abs()
    return {
...
@torch.no_grad()
def validate_soft_learner_noop(
    model,
    block,
    input_ids: torch.Tensor,
    x_in: torch.Tensor,
    ...
    """
    Check that replacing selected heads/positions with the explicit soft learner
    reproduces the cached teacher attention output.
    """
    if zcat_teacher.shape[0] != 1:
        raise ValueError("validate_soft_learner_noop assumes batch size 1")

    zcat_soft = zcat_teacher.clone()
    ...
    for pos in positions:
        for h in group:
            ...
            Kctx = k[:, h, : pos + 1, :]
            Vctx = v[:, h, : pos + 1, :]
            pred = LEARNER_REGISTRY.predict("soft", q_pos, Kctx, Vctx, cfg)[0]
            zcat_soft[0, pos, head_slice(h, head_dim)] = pred

    z_diff = (zcat_soft - zcat_teacher).abs()

    teacher_logits = continue_from_modified_block_gpt2(
        model=model,
        block=block,
        x_in=x_in,
        zcat_mod=zcat_teacher,
    ...
    soft_logits = continue_from_modified_block_gpt2(
        model=model,
        block=block,
        x_in=x_in,
        zcat_mod=zcat_soft,
    ...
    logit_diff = (teacher_logits - soft_logits).abs()

    return {
        "max_abs_z_diff": float(z_diff.max().item()),
        "mean_abs_z_diff": float(z_diff.mean().item()),
...
def evaluate_head_group(
    ...
    """
    Evaluate fixed and heterogeneous learner assignments for one sequence and
    one head group.
    """
    if input_ids.shape[0] != 1:
        raise ValueError("evaluate_head_group assumes batch size 1")

    # Baseline loss from the unmodified teacher attention output.
    logits_teacher = continue_from_modified_block_gpt2(
        model=model,
        block=block,
        x_in=x_in,
        zcat_mod=zcat_teacher,
    ...
    teacher_loss = float(mean_next_token_nll(logits_teacher, input_ids, positions).item())

    # Precompute learner predictions at each position/head.
    preds = {}
    for pos in positions:
        q_pos = {}
        ctx = {}
    ...
    # Local oracle: choose best assignment at each position in isolation,
    # then apply all local choices jointly.
...
def run_oracle_eval(cfg: SequenceOracleConfig):
    ...
    if not positions_template:
        raise ValueError("No evaluation positions. Check min_context, block_size, and position_stride.")

    # No-op validation on the first chunk and first head group.
    input_ids_0 = chunks[:1]
    x_in_0 = block_tensors["x_in"][:1].to(cfg.device)
    q_0 = block_tensors["q"][:1].to(cfg.device)
    k_0 = block_tensors["k"][:1].to(cfg.device)
    v_0 = block_tensors["v"][:1].to(cfg.device)
    zcat_teacher_0 = block_tensors["zcat_teacher"][:1].to(cfg.device)
    positions_0 = [p for p in positions_template if p < input_ids_0.shape[1] - 1]
    group_0 = head_groups[0]

    reconstruction_check = validate_noop_reconstruction(
        model=model,
        input_ids=input_ids_0,
        x_in=x_in_0,
        zcat_teacher=zcat_teacher_0,
        block=block,
        layer_idx=cfg.layer_idx,
    )
    soft_noop_check = validate_soft_learner_noop(
        model=model,
        block=block,
        input_ids=input_ids_0,
        x_in=x_in_0,
        ...
        zcat_teacher=zcat_teacher_0,
        group=group_0,
        positions=positions_0[: min(4, len(positions_0))],
        cfg=cfg,
        head_dim=head_dim,
    )

    print("[noop-check] reconstructed teacher logits")
    for k_name, value in reconstruction_check.items():
        print(f"  {k_name}: {value:.8e}")

    print("[noop-check] explicit soft learner replacement")
    for k_name, value in soft_noop_check.items():
        print(f"  {k_name}: {value:.8e}")
    ...
    for chunk_id in range(chunks.shape[0]):
        ...
        for group_idx, group in enumerate(head_groups):
            ...
            teacher_losses_all.append(torch.tensor([out["teacher_loss"]], dtype=torch.float32))
    ...
    return {
        ...
        "teacher_losses": torch.cat(teacher_losses_all, dim=0),
        ...
        "soft_noop_check": soft_noop_check,
    }

def summarize_to_dict(results: Dict[str, torch.Tensor]) -> Dict[str, object]:
    ...
    assignment_names = [candidate_name(assign) for assign in assignments]

    mean_fixed = fixed_losses.mean(dim=0)
    best_fixed_idx = int(mean_fixed.argmin().item())
    best_fixed_name = assignment_names[best_fixed_idx]
    best_fixed_loss = float(mean_fixed[best_fixed_idx].item())

    mean_teacher = float(teacher_losses.mean().item())
    mean_local_joint = float(local_joint_losses.mean().item())
    mean_greedy = float(greedy_losses.mean().item())

    fixed_assignment_losses = {
        name: float(mean_fixed[i].item())
        for i, name in enumerate(assignment_names)
    }

    winners = fixed_losses.argmin(dim=-1)
    winner_fractions = {
        name: float((winners == i).float().mean().item())
        for i, name in enumerate(assignment_names)
    }

    winner_fractions_by_unit = {}
    for unit_idx, unit in enumerate(units):
        mask = unit_ids == unit_idx
        w = winners[mask]
        unit_name = ",".join(map(str, unit))
        winner_fractions_by_unit[unit_name] = {
            name: float((w == i).float().mean().item())
            for i, name in enumerate(assignment_names)
        }
    ...
    return {
        ...
        "groups": units,
        "reconstruction_check": results.get("reconstruction_check", {}),
        "soft_noop_check": results.get("soft_noop_check", {}),
        "mean_teacher_loss": mean_teacher,
        "fixed_assignment_losses": fixed_assignment_losses,
        "best_fixed_name": best_fixed_name,
        "best_fixed_loss": best_fixed_loss,
        "mean_local_joint_loss": mean_local_joint,
        "mean_greedy_loss": mean_greedy,
        "best_fixed_minus_teacher": best_fixed_loss - mean_teacher,
        "local_joint_minus_teacher": mean_local_joint - mean_teacher,
        "greedy_minus_teacher": mean_greedy - mean_teacher,
        "best_fixed_minus_local_joint": best_fixed_loss - mean_local_joint,
        "best_fixed_minus_greedy": best_fixed_loss - mean_greedy,
        "winner_fractions": winner_fractions,
        "winner_fractions_by_unit": winner_fractions_by_unit,
    }


def summarize(results: Dict[str, torch.Tensor]) -> Dict[str, object]:
    summary = summarize_to_dict(results)

    print("\n=== No-op checks ===")
    print("Reconstructed teacher logits:")
    for k_name, value in summary["reconstruction_check"].items():
        print(f"  {k_name:<32} {value:.8e}")
    print("Explicit soft learner replacement:")
    for k_name, value in summary["soft_noop_check"].items():
        print(f"  {k_name:<32} {value:.8e}")

    print("\n=== Baseline ===")
    print(f"  {'teacher_loss':<32} {summary['mean_teacher_loss']:.6f}")

    print("\n=== Sequence-level fixed assignment benchmark ===")
    for name, loss in summary["fixed_assignment_losses"].items():
        print(f"  {name:<32} {loss:.6f}")
    print(
        f"  {'best_fixed':<32} "
        f"{summary['best_fixed_loss']:.6f} ({summary['best_fixed_name']})"
    )

    print("\n=== Sequence-level heterogeneous selectors ===")
    print(f"  {'local_then_joint':<32} {summary['mean_local_joint_loss']:.6f}")
    print(f"  {'greedy_suffix_oracle':<32} {summary['mean_greedy_loss']:.6f}")

    print("\n=== Gaps vs teacher ===")
    print(f"  {'best_fixed_minus_teacher':<32} {summary['best_fixed_minus_teacher']:.6f}")
    print(f"  {'local_joint_minus_teacher':<32} {summary['local_joint_minus_teacher']:.6f}")
    print(f"  {'greedy_minus_teacher':<32} {summary['greedy_minus_teacher']:.6f}")

    print("\n=== Gaps vs best fixed ===")
    print(f"  {'local_then_joint_gap':<32} {summary['best_fixed_minus_local_joint']:.6f}")
    print(f"  {'greedy_suffix_gap':<32} {summary['best_fixed_minus_greedy']:.6f}")

    print("\n=== Best fixed assignment winner fractions across examples ===")
    for name, frac in summary["winner_fractions"].items():
        print(f"  {name:<32} {frac:.3f}")

    print("\n=== Best fixed assignment winner fractions by unit ===")
    for unit_name, stats in summary["winner_fractions_by_unit"].items():
        stats_str = "  ".join(f"{k}:{v:.2f}" for k, v in stats.items())
        print(f"  group={unit_name:<12} {stats_str}")
...
def save_results(results: Dict[str, torch.Tensor], summary: Dict[str, object], cfg: SequenceOracleConfig) -> None:
    output_dir = Path(cfg.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)

    run_name = (
        f"layer{cfg.layer_idx}"
        f"__{cfg.replace_mode}"
        f"__g{cfg.head_group_size}"
        f"__stride{cfg.position_stride}"
        f"__chunks{cfg.max_chunks}"
        f"__seed{cfg.seed}"
    )

    raw_path = output_dir / f"{run_name}__raw.pt"
    summary_path = output_dir / f"{run_name}__summary.json"

    torch.save(results, raw_path)
...
def main():
    ...
    parser.add_argument(
        "--replace_mode",
        type=str,
        default="multi_head_single_pos_per_head",
    ...
    if cfg.save_results:
        save_results(results, summary, cfg)
